...
set_global_vars()
{
  ...
  if [[ -z "${__golang_image}" ]]; then
    ...
    __golang_image="google-go.pkg.dev/golang:${__golang_image_version}"
    log.debugvar __golang_image
  fi
...
package()
{
  ...
  KUBE_OUTPUT_SUBPATH="${__output_subpath}" \
    ...
    GOTOOLCHAIN="local" \
    GKE_EXTRA_DOCKER_REGISTRIES="gcr.io/gke-release-staging gcr.io/gke-on-prem-staging" \
  ...
  boringcrypto_hook
  # Note: Docker image building and license/source injection is now handled in gke/build/lib/release.sh
