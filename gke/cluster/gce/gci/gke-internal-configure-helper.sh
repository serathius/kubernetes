...
function start_internal_cluster_autoscaler {
  if [[ "${ENABLE_NAP:-}" == "true" || "${ENABLE_GKE_CLUSTER_AUTOSCALER:-}" == "true" ]]; then
    echo "Error: Requested NAP or cluster autoscaler, which should be configured via CRP. ENABLE_NAP=${ENABLE_NAP}, ENABLE_GKE_CLUSTER_AUTOSCALER=${ENABLE_GKE_CLUSTER_AUTOSCALER}"
    exit 1
  fi
}
...
EOF
  ...
  # Generate gvisor containerd shim config
    ...
    local -r gvisor_reproduce_nftables="${GVISOR_REPRODUCE_NFTABLES:-"false"}"
...
[runsc_config]
  ...
  reproduce-nftables = "${gvisor_reproduce_nftables}"
EOF
...
function gke-configure-npd-custom-plugins {
  ...
  GKE_NPD_CUSTOM_PLUGINS_CONFIG+=",${config_dir}/swap-monitor.json,${config_dir}/fs-readonly-monitor.json,${config_dir}/fs-capacity-monitor.json,${config_dir}/kubelet-monitor.json,${config_dir}/resource-monitor.json"
}
