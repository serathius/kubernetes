...
$EXEC_AUTH_PLUGIN_WINDOWS_AMD64_HASH="e4db43139e5a04b41350dab20cb1fa97db13f3bd066e0d831ccd2af0b317fb2f1c39a0f60e583daa3f2622af78f5b032e0537789f1d0e51020349010ed1d5d91"
...
function Start-GKEMetadataServer {
  ...
  if (Is-Antrea-Enabled $kube_env) {
    ...
    $listen_ip = Get_Endpoint_Gateway_From_CIDR ${env:POD_CIDR}
  }
...
function Get_MgmtNetAdapter {
  ...
  if (-not ${net_adapter}) {
    # If Add_InitialHnsNetwork() is not called, check the original adapter name
    $net_adapter = Get-NetAdapter | Where-Object Name -like "Ethernet*"
    if (-not ${net_adapter}) {
...
# Sets up the kubelet and starts it as native Windows service.
#
# Required ${kube_env} keys:
#   KUBELET_ARGS
function Start-Kubelet {
  ...
  WaitFor_ServiceReady "kubelet"
  Log-Output "Kubelet started successfully"
}

# Sets up the kube-proxy arguments and starts it as native
# Windows service.
#
# Required ${kube_env} keys:
#   KUBEPROXY_ARGS
...
function Start-Kubeproxy {
  ...
  # To perform the join in an automated way, it's preferable to apply the rename
  # and domain join in the GCESysprep step. However, after sysprep is complete
  # and the machine restarts, kube-proxy should not use the shortened
  # computer name, and instead use the instance's name by using --hostname-override,
  # otherwise kube-proxy will not be able to run properly.
  $instance_name = "$(Get-InstanceMetadata 'name' | Out-String)"
  ...
  WaitFor_ServiceReady "kube-proxy"
  Verify_GceMetadataServerRouteIsPresent
  Log-Output "Kubeproxy started successfully"
}

# Stop and unregister kubelet service.
function Unregister-Kubelet {
  & sc.exe delete kubelet
}

# Stop and unregister kube-proxy service.
function Unregister-Kubeproxy {
  & sc.exe delete kube-proxy
}

function WaitFor_ServiceReady($ServiceName) {
  $waited = 0
  $timeout = 10
  while (((Get-Service $ServiceName).Status -ne 'Running') -and $waited -lt $timeout) {
    Start-Sleep 1
    $waited++
  }
  ...
  # Timeout occurred
  if ($waited -ge $timeout) {
    Log-Output "$(Get-Service $ServiceName | Out-String)"
    Throw ("Timeout while waiting ${timeout} seconds for ${ServiceName} service to start")
  }
}
...
function Start-AntreaService {
  ...
  if ((Test-EnableWorkloadIdentity ${kube_env})) {
    Log-Output 'Running antrea-agent with --gke-wi-mode for workload identity'
    $antrea_args += "--gke-wi-mode"
  }
