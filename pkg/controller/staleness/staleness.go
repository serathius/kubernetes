...
type PodProber struct {
	...
	// podInformer is the shared informer
...
func (p *PodProber) runProbeLoop(ctx context.Context) {
	for {
		...
		// Create ListOptions with a selector that matches nothing.
		listOptions := metav1.ListOptions{
			FieldSelector: "metadata.name=",
		}
		resp, err := p.kubeClient.CoreV1().Pods("").List(ctx, listOptions)
		if err != nil {
...
func (p *PodProber) metricsLoop(ctx context.Context) {
	...
	for {
		...
		rv := p.podInformer.LastSyncResourceVersion()
...
func NewPodProber(ctx context.Context, client clientset.Interface, informer cache.SharedIndexInformer) *PodProber {
...
func (p *PodProber) QueryStaleness(podRV string) (time.Duration, error) {
	...
	for i := len(p.rvQueue) - 1; i >= 0; i-- {
		rvTime := p.rvQueue[i]
		cmp, err := resourceversion.CompareResourceVersion(rvTime.rv, podRV)
		if err != nil {
			return 0, err
		}
		if cmp <= 0 {
			return time.Since(rvTime.now), nil
		}
	}
