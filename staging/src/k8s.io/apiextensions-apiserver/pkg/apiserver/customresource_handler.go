...
func (r *crdHandler) getOrCreateServingInfoFor(uid types.UID, name string) (*crdInfo, error) {
	...
	for _, v := range crd.Spec.Versions {
			...
			supportedMediaTypes: []runtime.SerializerInfo{
				{
					...
					EncodesAsText:    true,
					Serializer:       json.NewSerializerWithOptions(json.DefaultMetaFactory, creator, typer, json.SerializerOptions{StreamingCollections: streamingCollections}),
					...
					StrictSerializer: json.NewSerializerWithOptions(json.DefaultMetaFactory, creator, typer, json.SerializerOptions{
						Strict:               true,
						StreamingCollections: streamingCollections,
					}),
		...
		if utilfeature.DefaultFeatureGate.Enabled(features.StreamingCollectionEncodingToJSON) {
			opts = append(opts, serializer.WithStreamingCollectionEcoderToJson())
		}
