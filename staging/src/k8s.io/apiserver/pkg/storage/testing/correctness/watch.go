...
func ValidateWatchGuarantees(t *testing.T, versioner storage.Versioner, history *WatchHistory, rec RecordedWatch) {
	t.Helper()
	if versioner == nil {
	...
	t.Logf("Watch[%s]: %d total events read (startRV=%s, sendInitialEvents=%t) -> [Added: %d, Modified: %d, Deleted: %d, Bookmarks: %d, Errors: %d]",
		watchName, len(events), rec.ResourceVersion, rec.SendInitialEvents, added, modified, deleted, bookmarks, errors)

	if rec.SendInitialEvents {
		validateWatchListStream(t, versioner, history, rec, events, watchName)
		return
	}
	validateStandardWatchStream(t, versioner, history, rec, events, watchName)
}

func validateStandardWatchStream(t *testing.T, versioner storage.Versioner, history *WatchHistory, rec RecordedWatch, events []watch.Event, watchName string) {
	t.Helper()
	...
	startRV, _ := versioner.ParseResourceVersion(rec.ResourceVersion)
	expectedMatchingEvents := history.ExpectedEvents(rec.Prefix, startRV, rec.Predicate)

	require.True(t, len(mutatingEvents) <= len(expectedMatchingEvents),
		"watch[%s] received more events (%d) than expected matching events (%d)",
		watchName, len(mutatingEvents), len(expectedMatchingEvents))

	for i := range mutatingEvents {
		exp := expectedMatchingEvents[i]
...
func validateWatchListStream(t *testing.T, versioner storage.Versioner, history *WatchHistory, rec RecordedWatch, events []watch.Event, watchName string) {
	t.Helper()
	...
	recStreaming := rec
	recStreaming.ResourceVersion = fmt.Sprintf("%d", bookmarkRV)
	recStreaming.SendInitialEvents = false
	validateStandardWatchStream(t, versioner, history, recStreaming, streamingEvents, watchName+"-streaming")
}
...
func PodAttrFunc(obj runtime.Object) (labels.Set, fields.Set, error) {
	...
	if !ok {
		return storage.DefaultClusterScopedAttr(obj)
	}
