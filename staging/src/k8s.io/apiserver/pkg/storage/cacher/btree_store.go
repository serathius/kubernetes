...
func (si *threadedStoreIndexer) Delete(obj interface{}) error {
	...
	si.lock.Lock()
	defer si.lock.Unlock()
	oldObj, err := si.store.deleteElem(storeElem)
	if err != nil {
		return err
	}
...
func (s *btreeStore) Delete(obj interface{}) error {
	...
	_, err := s.deleteElem(storeElem)
	return err
}

func (s *btreeStore) deleteElem(storeElem *storeElement) (interface{}, error) {
	item := s.tree.Delete(storeElem)
	if item == nil {
		return nil, fmt.Errorf("obj does not exist")
	}
	return item, nil
}
...
func (c *continueCache) Get(rv uint64) (orderedStore, bool) {
	c.RLock()
	defer c.RUnlock()
