...
import (
	...
	keypkg "k8s.io/apiserver/pkg/storage/cacher/key"
...
func (w *WatchCacheStorage) SnapshotLocked() Snapshot {
	return w.store
}
...
func (l ListSnapshot) OrderedListPrefix(prefix string, continueKey string) ([]interface{}, error) {
	...
	for _, item := range l.Items {
		...
		if !keypkg.HasPathPrefix(elem.Key, prefix) {
			continue
		}
...
func (w *WatchCacheStorage) UpdateStoreLocked(eventType watch.EventType, elem *Element) error {
	switch eventType {
	case watch.Added:
