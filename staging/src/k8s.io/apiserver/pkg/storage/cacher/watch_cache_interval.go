...
func newCacheIntervalFromStore(resourceVersion uint64, indexer cacherstore.Indexer, key string, matchesSingle bool) (*watchCacheInterval, error) {
	buffer := &watchCacheIntervalBuffer{}
	...
	for i, item := range allItems {
		elem, ok := item.(*cacherstore.Element)
		if !ok {
