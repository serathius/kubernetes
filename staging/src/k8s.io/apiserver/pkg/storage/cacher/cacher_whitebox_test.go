...
func TestTooManyRequestsNotReturned(t *testing.T) {
	...
	backingStorage := &cachertesting.DummyStorage{GetListErr: dummyErr}
	cacher, _, err := newTestCacherWithoutSyncing(backingStorage, clock.RealClock{})
	if err != nil {
		t.Fatalf("Couldn't create cacher: %v", err)
	}
...
func TestWatchNotHangingOnStartupFailure(t *testing.T) {
	...
	dummyErr := fmt.Errorf("dummy")
	backingStorage := &cachertesting.DummyStorage{WatchErr: dummyErr}
...
func TestWaitUntilWatchCacheFreshAndForceAllEvents(t *testing.T) {
	...
	scenarios := []struct {
		...
		opts               storage.ListOptions
		backingStorage     *cachertesting.DummyStorage
		verifyBackingStore func(t *testing.T, s *cachertesting.DummyStorage)
	}{
		{
...

		{
			...
			backingStorage: func() *cachertesting.DummyStorage {
				hasBeenPrimed := false
				s := &cachertesting.DummyStorage{}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			t.Parallel()
			var backingStorage *cachertesting.DummyStorage
...
func (m fakeStorage) Watch(_ context.Context, _ string, _ storage.ListOptions) (watch.Interface, error) {
	return cachertesting.NewDummyWatch(), nil
}
...
func (f fakeOrderedLister) ListPrefix(prefix, continueKey string) []interface{} {
	return nil
}

func (f fakeOrderedLister) Count(prefix, continueKey string) int {
	return 0
}

func (f fakeOrderedLister) Clone() store.OrderedLister {
	return f
}
