...
func (s *store) conditionalDelete(
	...
	for {
		...
		startTime := time.Now()
		txnResp, err := s.client.Kubernetes.OptimisticDelete(ctx, key, clientv3.DeleteOptions{ExpectedRevision: origState.rev})
...
func (s *store) GuaranteedUpdate(
	...
	for {
		...
		txnResp, err := s.client.Kubernetes.OptimisticPut(ctx, preparedKey, newData, clientv3.PutOptions{ExpectedRevision: origState.rev, LeaseID: lease})
