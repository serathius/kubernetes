...
func StartCompactor(client *clientv3.Client, compactInterval time.Duration) Compactor {
	...
	c := newCompactor(client, compactInterval, clock.RealClock{})
	for _, ep := range client.Endpoints() {
		endpointsMap[ep] = c
	}
	return c
...
func newCompactor(client *clientv3.Client, compactInterval time.Duration, clock clock.Clock) *compactor {
	c := &compactor{
		client:   client,
...
func (c *compactor) UpdateInterval(interval time.Duration) {
	if interval <= 0 {
		return
	}
	c.mux.Lock()
