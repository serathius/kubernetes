...
func randPod(b *testing.B, pod *v1.Pod, size int, r *rand.Rand) {
	...
	pod.Spec.Hostname = rand2.String(size)
...
func benchmarkItems(b *testing.B, file string, size, n int) *v1.PodList {
	pod := v1.Pod{}
	f, err := os.Open(file)
	if err != nil {
...
func benchmarkSerializeObject(b *testing.B, header http.Header, encoder runtime.Encoder, object runtime.Object) {
	req := &http.Request{
		Header: header,
		URL: &url.URL{Path: "/path"},
	}
	...
	for i := 0; i < b.N; i++ {
		recorder := httptest.NewRecorder()
		SerializeObject("application/json", encoder, recorder, req, http.StatusOK, object)
		result := recorder.Result()
...
func BenchmarkSerializeObject(b *testing.B) {
	...
	f, err := os.Stat("testdata/pod.json")
	if err != nil {
		b.Fatalf("Cannot open file: %s", err)
	}
	fileSize := f.Size()
	...
	for _, countSize := range countSize {
				b.Run(fmt.Sprintf("Count=%d/Size=%d", countSize.count, countSize.size), func(b *testing.B) {
					items := benchmarkItems(b, "testdata/pod.json", int(countSize.size-fileSize), countSize.count)
					jsonData := toJSON(b, items)
					protoData := toProtoBuf(b, items)
					for _, header := range headers {
						b.Run(fmt.Sprintf("Header=%s", header.name), func(b *testing.B) {
							b.Run("Format=ProtoBuf", func(b *testing.B) {
								b.Run("Encoder=Fake", func(b *testing.B) {
									encoder := newFakeEncoder(b, protoData)
									benchmarkSerializeObject(b, header.Header, encoder, nil)
								})
								b.Run("Encoder=Real", func(b *testing.B) {
									encoder := protobuf.NewSerializer(nil, nil)
									benchmarkSerializeObject(b, header.Header, encoder, items)
								})
							})
							b.Run("Format=JSON", func(b *testing.B) {
								b.Run("Encoder=Fake", func(b *testing.B) {
									encoder := newFakeEncoder(b, jsonData)
									benchmarkSerializeObject(b, header.Header, encoder, nil)
								})
								b.Run("Encoder=Real", func(b *testing.B) {
									encoder := json2.NewSerializerWithOptions(nil, nil, nil, json2.SerializerOptions{})
									benchmarkSerializeObject(b, header.Header, encoder, items)
								})
								b.Run("Encoder=Streaming", func(b *testing.B) {
									encoder := json2.NewSerializerWithOptions(nil, nil, nil, json2.SerializerOptions{StreamingCollectionsEncoding: true})
									benchmarkSerializeObject(b, header.Header, encoder, items)
								})
							})
						})
...
func newFakeEncoder(b *testing.B, payload []byte) *fakeEncoder {
