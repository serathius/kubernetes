...
func cacheKeyFunc(obj runtime.Object) (string, error) {
	...
	if len(pod.Namespace) == 0 {
		return "/pods/" + pod.Name, nil
	}
	return "/pods/" + pod.Namespace + "/" + pod.Name, nil
}

func newEtcdStorageForResource(t *testing.T, resource schema.GroupResource) *storagebackend.ConfigForResource {
	t.Helper()

	completedConfig := kubeapiserver.NewStorageFactoryConfig().Complete(options.NewEtcdOptions(framework.SharedEtcd()))
...
func setupStore(t *testing.T, decorator generic.StorageDecorator) (storage.Interface, string) {
	...
	storageConfig := newEtcdStorageForResource(t, schema.GroupResource{Resource: "pods"})
	...
	return store, storageConfig.Prefix
}
